...
def event_post():
    ...
    if filter_option == 'today':
        query += " WHERE DATE(Event_date) = DATE(NOW())"
    elif filter_option == 'this_week':
        query += " WHERE WEEK(Event_date) = WEEK(NOW()) AND YEAR(Event_date) = YEAR(NOW())"
    elif filter_option == 'this_month':
        query += " WHERE MONTH(Event_date) = MONTH(NOW()) AND YEAR(Event_date) = YEAR(NOW())"

    # Fetch filtered or all events
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)
    ...
    return render_template('event_post.html', posts=posts, filter_option=filter_option)
